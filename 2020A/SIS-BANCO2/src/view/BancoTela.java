...
public class BancoTela {
	...
	public void cadastrarConta() {
		...
		tipoConta = JOptionPane
				.showInputDialog("Informe tipo de conta a ser criada:\n\n" + "[1] Poupança\n" + "[2] Corrente\n");

//		 if (!tipoConta.equalsIgnoreCase("p") || !tipoConta.equalsIgnoreCase("c")) {
//
//			tipoConta = JOptionPane.showInputDialog(
//					"Informe tipo de conta a ser criada:\n\n" + "[ P ] Poupança\n" + "[ C ] Corrente\n");
//		}

		/* CRIA CONTA POUPANCA */
		if (Integer.parseInt(tipoConta) == 1) {
			if (cliente != null) {
				ContaPoupanca contaPoupanca = (ContaPoupanca) new ContaPoupanca.Builder(cliente).build();
		...
		/* CRIA CONTA CORRENTE */
		if (Integer.parseInt(tipoConta) == 2) {
			if (cliente != null) {
