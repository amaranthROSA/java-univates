 ...
 *
 * @version 1.3 Modified (Aug 12, 2003): Portuguese version, added methods
 *          without parameters.
 */
...
public class Entrada {
	 ...
	 ** 
	 * @param prompt       the message string to be displayed inside dialog
	 ** @param initialValue input string that is initially displayed as selected by
	 *                     the user
	 ** @return String input from the user.
	 **/
	public static String leiaString(String prompt, String initialValue) {
		...
		while (!validResponse) {
			final JOptionPane optionPane = new JOptionPane(commentArray, JOptionPane.QUESTION_MESSAGE,
	 ...
	 ** 
	 * @param prompt       the message string to be displayed inside dialog
	 ** @param initialValue input value that is initially displayed as selected by
	 *                     the user
	 ** @return the input character
	 **/
	...
	public static char leiaChar(String prompt, String initialValue) {
		char response = (initialValue != null && initialValue.length() > 0) ? initialValue.charAt(0) : '-'; // EA:
																											// modified
		...
		while (!validResponse) {
			...
			if (input != JOptionPane.UNINITIALIZED_VALUE) {
				...
				if (result != null) {
					...
					commentArray[1] = "Entrada inválida"; // EA: corrected, no point to print null-object. Question:
															// when it is possible to have null objects?
					commentArray[2] = "Entre com apenas um caracter";
				}
			} else {
	...
	public static boolean leiaBoolean(String prompt, String trueText, String falseText) {
		...
		while (!validResponse) {
			...
			result = JOptionPane.showOptionDialog(null, commentArray, BOOLEAN_TITLE, JOptionPane.YES_NO_OPTION,
